...
window.onload = function () {
    ...
    var url = urlBase + "Mantenimiento/listarArticulo/?ss=" + ss;
...
function listarArticulo(irUltimaPagina) {
    crearMatriz();
...
function crearTabla() {
    ...
    contenido += "</tr>";
    contenido += "</thead>";
    contenido += "<tbody id='tbArticulo' class='tabla-FilaCuerpo'>";
    contenido += "</tbody>";
    contenido += "<tfoot>";
    ...
    contenido += "</tfoot>";
    contenido += "</table>";
    document.getElementById("divArticulo").innerHTML = contenido;
}
...
function crearMatriz() {
    ...
    for (i = 0; i < nRegistros; i++) {
        ...
        for (j = 0; j < nCampos; j++) {
            if (isNaN(campos[j]) || j==0) matriz[x][j] = campos[j];
...
function mostrarMatriz(indicePagina) {
    ...
    if (n > 0) {
        ...
        for (var i = inicio; i < fin; i++) {
            if (i < n) {
                ...
                contenido += "<td style='text-align:center'><input type='radio' name='rdnArticulo' id='rdn";
                contenido += i;
    ...
    document.getElementById("tbArticulo").innerHTML = contenido;
    crearPaginas();
    configurarChecks();
}
...
function configurarControles() {
    var btnSeleccionarArticulo = document.getElementById("btnSeleccionarArticulo");
    btnSeleccionarArticulo.onclick = function () {
        if (idPasar != "") {
            var hdfArt = window.parent.document.getElementById("hdfArt");
            hdfArt.value = idPasar;
            hdfArt.onchange();
            window.parent.abrirPopup('PopupArticulo');
            limpiar();
        }
        else {
            return false;
    ...
    btnCancelarArticulo.onclick = function () {
        limpiar();
        window.parent.abrirPopup('PopupArticulo');
    }
}
...
function configurarChecks() {
    var rdnComponente = document.getElementsByName("rdnArticulo");

...
function listarComponente(irUltimaPagina) {
    crearMatriz();
...
function crearTabla() {
    ...
    contenido += "</tr>";
    contenido += "</thead>";
    contenido += "<tbody id='tbComponente' class='tabla-FilaCuerpo'>";
    contenido += "</tbody>";
    contenido += "<tfoot>";
    ...
    contenido += "</tfoot>";
    contenido += "</table>";
    document.getElementById("divComponente").innerHTML = contenido;
}
...
function mostrarMatriz(indicePagina) {
    ...
    if (n > 0) {
        ...
        for (var i = inicio; i < fin; i++) {
            if (i < n) {
                ...
                contenido += "<td style='text-align:center'><input type='radio' name='rdnComponente' id='rdn";
                contenido += i;
    ...
    document.getElementById("tbComponente").innerHTML = contenido;
    crearPaginas();
    configurarChecks();
}
...
function configurarControles() {
    var btnSeleccionarComponente = document.getElementById("btnSeleccionarComponente");
    btnSeleccionarComponente.onclick = function () {
        if (idPasar != "") {
            var hdfCom = window.parent.document.getElementById("hdfCom");
            hdfCom.value = idPasar;
            hdfCom.onchange();            
            window.parent.abrirPopup('PopupComponente');
            limpiar();
        }
        else {
            return false;
    ...
    var btnCancelarComponente = document.getElementById("btnCancelarComponente");
    btnCancelarComponente.onclick = function () {
        limpiar();
        window.parent.abrirPopup('PopupComponente');
    }
}
...
function configurarChecks() {
    var rdnComponente = document.getElementsByName("rdnComponente");
    var nCampos = rdnComponente.length;
    for (var x = 0; x < nCampos; x++) {
        rdnComponente[x].onclick = function () {
...
function limpiar() {
    ...
    var nCampos = rdnComponente.length;
    for (var x = 0; x < nCampos; x++) {
        if (rdnComponente[x].checked){
            rdnComponente[x].checked = false;
            break;
        }
    }
    var cabecera = document.getElementsByClassName("Texto");
    for (var y = 0; y < cabecera.length; y++) {
    	cabecera[y].value = "";
    }
    filtrar();
    idPasar = "";

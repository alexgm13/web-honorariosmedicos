var lista = [];
var matriz = [];
var cabeceras = ["Código", "Apellido Paterno", "Apellido Materno", "Nombres"];
var anchos = [10, 30, 30, 30];
var matrizIndice = [0, 1, 2, 3];
...
window.onload = function () {
    ...
    var contenido = "";
    document.getElementById("txtBusquedaNombre").focus();
    ...
    document.getElementById("tbPaciente").innerHTML = contenido;
}
...
function listarPaciente(irUltimaPagina) {
    crearMatriz();
...
function crearTabla() {
    ...
    contenido += "</tr>";
    contenido += "</thead>";
    contenido += "<tbody id='tbPaciente' class='tabla-FilaCuerpo'>";
    contenido += "</tbody>";
    contenido += "<tfoot>";
    ...
    contenido += "</tfoot>";
    contenido += "</table>";
    document.getElementById("divPaciente").innerHTML = contenido;
}
    ...
    if (n > 0) {
        ...
        for (var i = inicio; i < fin; i++) {
            if (i < n) {
                ...
                contenido += "<td style='text-align:center'><input type='radio' name='rdnPaciente' id='rdn";
                contenido += i;
...
function configurarControles() {
    var btnSeleccionarPaciente = document.getElementById("btnSeleccionarPaciente");
    btnSeleccionarPaciente.onclick = function () {
        if (idPasar != "") {
            var hdfIdPaciente = window.parent.document.getElementById("hdfIdPaciente");
            hdfIdPaciente.value = idPasar;
            hdfIdPaciente.onchange();
            limpiar();
            window.parent.abrirPopup('PopupPaciente');
            matriz = [];
            var contenido = "";
            document.getElementById("txtBusquedaNombre").focus();
            var nCabeceras = cabeceras.length;
    ...
    btnBuscar.onclick = function () {
        var txtBusquedaApePaterno = document.getElementById("txtBusquedaApePaterno").value;
        var txtBusquedaApeMaterno = document.getElementById("txtBusquedaApeMaterno").value;
        var txtBusquedaNombre = document.getElementById("txtBusquedaNombre").value;
        if (txtBusquedaApePaterno == "" && txtBusquedaApeMaterno == "" && txtBusquedaNombre == "") {
            window.parent.mostraralerta("Seleccione algún criterio de busqueda");
        }
        else {
            var strDatos = txtBusquedaApePaterno + "|" + txtBusquedaApeMaterno + "|" + txtBusquedaNombre;
            var url = urlBase + "Proceso/listarPacientes/?ss=" + ss;
...
function configurarChecks() {
    var rdnPaciente = document.getElementsByName("rdnPaciente");
    var nCampos = rdnPaciente.length;
    for (var x = 0; x < nCampos; x++) {
        rdnPaciente[x].onclick = function () {
...
function limpiar() {
	document.getElementById("txtBusquedaApePaterno").value = "";
	document.getElementById("txtBusquedaApeMaterno").value = "";
	document.getElementById("txtBusquedaNombre").value = "";
	
    var rdnPaciente = document.getElementsByName("rdnPaciente");
    var nCampos = rdnPaciente.length;
    for (var x = 0; x < nCampos; x++) {
        if (rdnPaciente[x].checked) {
            rdnPaciente[x].checked = false;
            break;
        }
    }
    idPasar = "";

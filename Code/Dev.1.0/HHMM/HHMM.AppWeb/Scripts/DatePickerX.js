            ...
            input.addEventListener('click', function(e)
            {
                if (!isActive()) {
                    ...
                    if (input.nodeName == "INPUT") {
                    	...
                    	var top = datepicker.bottom-15;
                    	if (datepicker) {
                    		var ifrm = window.top.document.getElementById("ifrPrincipal");
                    		var alto = ifrm.getBoundingClientRect().height;
                    		var ancho = ifrm.getBoundingClientRect().width;
                    		var scrollTop = 0;
                    		if (ifrm != null) {
                    			var doc = ifrm.contentDocument.children[0];
                    			scrollTop = doc.children[1].scrollTop;
                    		}
                    		
                    		var diferencia = (alto - (top + scrollTop)) < 255 ? (top = top - (255 + datepicker.height)) : "";
                    		var diferencia = (ancho - left) < 230 ? (left = left - (230)) : "";
